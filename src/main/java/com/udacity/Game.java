...
//        Horizontal check
        for (int i = 0; i < size_grid; i++) {
            Character grid_value = grid[0][i];
            if (grid_value == 'x' && grid_value.equals(grid[1][i]) && grid_value.equals(grid[2][i])) {
                result = "X wins";
                return result;
            } else if (grid_value == 'o' && grid_value.equals(grid[1][i]) && grid_value.equals(grid[2][i])) {
                result = "O wins";
                return result;
            }
        }
//        Vertical check
        for (int j = 0; j < size_grid; j++) {
            Character grid_value = grid[j][0];
            if (grid_value == 'x' && grid_value.equals(grid[j][1]) && grid_value.equals(grid[j][2])) {
                result = "X wins";
                return result;
            } else if (grid_value == 'o' && grid_value.equals(grid[j][1]) && grid_value.equals(grid[j][2])) {
                result = "O wins";
                return result;
            }
        }
//         Diagonal check
        char grid_value = grid[1][1];
        switch (grid_value) {
            case 'x':
                if ((grid_value == grid[0][0] && grid_value == grid[2][2]) ||
                        (grid_value == grid[0][2] && grid_value == grid[2][0])) {
                    result = "X wins";
                }
                break;
            case 'o':
